...
export default function Blog() {
  ...
  return (
    <main>
      <section className='our-blog grid-bg pt-5'>
        <div className="container">
          <div className='d-flex justify-content-center mt-5'>
            <span className="about-badge"> <i className='fas fa-newspaper'></i>مدونتنا</span>
          </div>
          <h1 className="blog-title">
            اكتشف <span>مقالاتنا</span>
          </h1>
          <p className='blog-content d-flex justify-content-center pt-2 m-auto'>
      ...
      <section className='search'>
        <div className="container d-flex justify-content-between py-3 text-center">
          ...
          <div className="filter-buttons d-flex justify-content-center gap-2 mt-1">
            <button className={`filter-btn ${selectedCategory === 'all' ? 'active' : ''}`} onClick={() => handleCategory('all')}>جميع المقالات</button>
            <button className={`filter-btn ${selectedCategory === 'إضاءة' ? 'active' : ''}`} onClick={() => handleCategory('إضاءة')}>إضاءة</button>
            <button className={`filter-btn ${selectedCategory === 'بورتريه' ? 'active' : ''}`} onClick={() => handleCategory('بورتريه')}>بورتريه</button>
            <button className={`filter-btn ${selectedCategory === 'مناظر طبيعية' ? 'active' : ''}`} onClick={() => handleCategory('مناظر طبيعية')}>مناظر طبيعية</button>
            <button className={`filter-btn ${selectedCategory === 'تقنيات' ? 'active' : ''}`} onClick={() => handleCategory('تقنيات')}>تقنيات</button>
            <button className={`filter-btn ${selectedCategory === 'معدات' ? 'active' : ''}`} onClick={() => handleCategory('معدات')}>معدات</button>
          </div>
        </div>
      </section>

      <section className='display-artic pt-5'>
        <div className="container">
          <div className="row align-items-center justify-content-between">
            <div className="col-auto">
              <p className="show-articles">عرض <span>{filteredArticles.length}</span> مقالات</p>
            </div>

            <div className="col-auto">
              <div className="display-way d-flex">
                <button className={`view-btn ${activeView === 'grid' ? 'active' : ''}`} onClick={() => setActiveView('grid')}>
                  <i className="fas fa-th-large"></i>
                </button>
                <button className={`view-btn ${activeView === 'list' ? 'active' : ''}`} onClick={() => setActiveView('list')}>
      ...
      <section className="posts-section pt-4">
        <div className="container">
          <div className={`row g-4 ${activeView === 'list' ? 'flex-column' : ''}`}>
            {displayedArticles.map((post) => (
              <div className={activeView === 'grid' ? "col-md-4" : "col-12"} key={post.id}>
                <Link to={'/blog-details/' + post.id} className="post-link">
          ...
          <div className="pagination d-flex justify-content-center gap-2 mt-5">
          ...
          <p className='text-center'>
            أنت في الصفحة {currentPage} من {totalPages}
          </p>
        </div>
      </section>
    </main>
